...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/08/20 17:34:25 by schennal          #+#    #+#             */
/*   Updated: 2024/08/20 17:34:33 by schennal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	BitcoinExchange::validateInputValue(std::string &line, std::string &value)
{
	...
	for (int i = 1; i < (int)value.size(); i++)
	{
		...
		if ((value[i] != '.' && !std::isdigit(value[i])) || count > 1)
		{
			std::cout << "Error: bad input" << " => {" << line << "}" << std::endl;
			return (0);
		}
...
void	BitcoinExchange::processValidInput(std::string &date, std::string &valueStr)
{
	...
	{
		double valueDouble;
		std::istringstream(value) >> valueDouble;
		if (valueDouble > 1000)
		{
